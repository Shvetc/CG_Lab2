...
namespace tomogram_visualizer
{
    public partial class Form1 : Form
    {
        ...
        class Bin // чтение данных файлов
        {
        ...
        class View //  класс, содержащий функции  для визуализации томограммы 
        {
            ...
            Color TransferFunction(short value, int min, int max_min) // функция  перевода  значения  плотностей  томограммы  в  цвет
            {

                int newVal = clamp((value - min) * 255 / (max_min), 0, 255);
                return Color.FromArgb(255, newVal, newVal, newVal);
            }
            public void DrawQuads(int layerNumber, int min, int maxmin) // отрисовка четырехугольника 
            {
            ...
            Bitmap textureImage; 
            int VBOTexture; // номер текстуры 
            public void Load2DTexture() // загрузка текстуры в память  видеокарты
            {
                GL.BindTexture(TextureTarget.Texture2D, VBOTexture); // связывает    текстуру,  делает  ее  активной,  а  также указывает  ее  тип
            ...
            //визуалиация томограммы одним прямоугольником
            public void genereateTextureImage(int layerNumber, int min, int max_min) //генерирование изображения из томограммы при помощи созданной TransferFunction
            {
            ...
            public void DrawTexture() //включение  2d-текстурированя/выбор  текстуры/ Рисование  одного  прямоугольника  с наложенной текстурой/выключение 2d-текстурирования
            {
                GL.Clear(ClearBufferMask.ColorBufferBit | ClearBufferMask.DepthBufferBit);
        ...
        int currentLayer; //  переменная типа int, которая хранит номер слоя для визуализации.
        private void openToolStripMenuItem_Click(object sender, EventArgs e) // открытие файла с томограммой и настройкой его визуализации
        {
        ...
        private void glControl1_Paint(object sender, PaintEventArgs e)
        {
            if (loaded)
            {
                ...
                if (radioButton2.Checked == true)
                {
                    ...
                    view.DrawQuads(currentLayer, trackBar2.Value, trackBar3.Value);
                    glControl1.SwapBuffers(); //// загрузка в буфер буфер экрана
                }
            ...
            {
                currentLayer = trackBar1.Value; // присвоение значению trackBar1  количества слоев 
                needReload = true;
            }
